    ...
    for y, row in ipairs(piece) do
        for x, cell in ipairs(row) do
            if cell ~= 0 then
                ...
                if px < 0 or px >= width or py >= height or (py >= 0 and pixels[1 + px + (width * py)].color ~= colors[0]) or (py < 0) then
                    print('False.')
                    return false
                end
            end
...
local function remove_full_lines()
    for y = 0, height - 1 do
        ...
        for x = 1, width do
            if pixels[1 + x + (width * y)].color == colors[0] then
                is_full_line = false
                break
            end
        end
        if is_full_line then
            ...
            for remove_y = y, height - 1 do
                for x = 1, width do
                    pixels[1 + x + (width * remove_y)].color = pixels[1 + x + (width * (remove_y + 1))].color
                end
            end
            -- Clear the top line
            for x = 1, width do
                pixels[1 + x + (width * height)].color = colors[0]
            end
